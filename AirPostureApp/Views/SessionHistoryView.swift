...
class SessionHistoryViewModel: ObservableObject {
    ...
    @Published private(set) var averageGoodPosture: Int = 0
    ...
    private func updateStats() {
        ...
        averageGoodPosture = total / visibleSessions.count
...
struct SessionData: Identifiable {
    ...
    let runningWalkingDuration: TimeInterval
    let runningWalkingPercentage: Double
    ...
    init(from session: Session) {
        ...
        self.avatarType = session.avatarType
        self.runningWalkingDuration = session.runningWalkingDuration
        self.runningWalkingPercentage = session.runningWalkingPercentage
    }
}
...
struct SessionHistoryView: View {
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.selectedTimeframe) {
            AnalyticsManager.shared.logEvent("history_timeframe_changed", parameters: [
                "timeframe": viewModel.selectedTimeframe.rawValue
            ])
            viewModel.handleTimeframeChange()
        }
        .onAppear {
            AnalyticsManager.shared.logScreenView(screenClass: "SessionHistoryView", screenName: "Session History")
            Task {
                await viewModel.refreshSessions()
            }
        }
    ...
    private var headerView: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !viewModel.visibleSessions.isEmpty {
                Text("Average good posture: \(viewModel.averageGoodPosture)%")
        ...
        List {
            Section(header: Text("Recent Sessions")) {
                ForEach(viewModel.visibleSessions) { session in
                    HStack(spacing: 12) {
                        // Session avatar - use bear-running if active >50% of session
                        Image(session.runningWalkingPercentage > 50 ? "bear-running" : session.avatarType)
                            ...
                            .background(
                                Circle()
                                    .fill(session.poorPosturePercentage > 50 ? PostureColors.poor.opacity(0.1) : session.poorPosturePercentage >= 40 ? PostureColors.warning.opacity(0.1) : PostureColors.good.opacity(0.1))
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                ...
                                Text("\(session.poorPosturePercentage)%")
                                    .foregroundColor(session.poorPosturePercentage > 50 ? PostureColors.poor : session.poorPosturePercentage >= 40 ? PostureColors.warning : PostureColors.good)
                                    .fontWeight(.medium)
                                    .animation(.easeInOut(duration: 0.3), value: session.poorPosturePercentage)
                            }
